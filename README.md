<h1 align="center">Vikram Samak</h1>
<p align="center">
Automation Engineer · Workflow Architect · Full Stack Developer
</p>

---

### About Me
- Automation-focused engineer building scalable and reliable workflows
- Strong experience with n8n, API integrations, webhooks, and JSON
- Working on AI-powered automation using MCP, RAG, and LLM workflows
- Full stack development background that helps design robust automation systems
- Focused on reducing manual work through clean and maintainable automation

---

### What I Work On
- End-to-end workflow automation using n8n
- API and webhook-based system integrations
- AI-driven automation pipelines for data and content
- Backend logic for automation platforms
- Automation-first system and product design

---

### Portfolio & Contact
- Portfolio: https://vikramsamak.com  
- Email: vikramsamak02@gmail.com

---

### Connect With Me
<p align="left">
  <a href="https://linkedin.com/in/vikram-samak">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  &nbsp;&nbsp;
  <a href="https://instagram.com/vikramsamak">
    <img src="https://img.shields.io/badge/Instagram-Follow-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>
</p>

---

### Tech & Tools
<p>
  <img src="https://cdn.simpleicons.org/n8n/ea4b71" height="36" />
  &nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/javascript/f7df1e" height="36" />
  &nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/typescript/3178c6" height="36" />
  &nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/nodedotjs/339933" height="36" />
  &nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/mongodb/47a248" height="36" />
  &nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/postman/ff6c37" height="36" />
  &nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/react/61dafb" height="36" />
  &nbsp;&nbsp;
  <img src="https://cdn.simpleicons.org/nextdotjs/ffffff" height="36" />
</p>

---

<p align="center">
Automate · Optimize · Scale
</p>
